...
class ImageEmbddingLoss(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.feature=CLIPVisualEncoder(self.model)
        #self.Convfeature=ResNetEncoder(self.model1)
    ...
    def forward(self, masked_generated, masked_img_tensor):
        ...
        #g1,g2 = self.Convfeature(masked_generated)
      
        g = self.feature(masked_generated)
        ...
        #t1,t2 = self.Convfeature(masked_generated)
        
        t = self.feature(masked_img_tensor)
        
        cos_target = torch.ones((masked_img_tensor.shape[0], 1)).float().cuda()
        #similarity = 2*self.cosloss(g, t, cos_target).unsqueeze(0) + self.mse(g1, t1) + self.mse(g2, t2)
        similarity = self.cosloss(g, t, cos_target).unsqueeze(0) + self.mse(masked_generated_feature1, masked_img_tensor_feature1) + self.mse(masked_generated_feature11, masked_img_tensor_feature11)
